    ...
    interface IPlanner with
        ...
        member this.Scale (policy: IPolicy) (plan: IPlan) =
            let PlanVertexLoad v = 
                plan.InEdges v |> Seq.map (fun e -> e.Tag.Load)
                               |> Seq.sum

            let PolicyVertexLoads pv = 
                plan.FindPlanVertices pv |> Seq.map (fun v -> PlanVertexLoad v)
                                         |> Seq.sum

            let Scale pv = 
                let totalLoad = PolicyVertexLoads pv

                let replicaNumIdeal = totalLoad * pv.UnitCore |> ceil |> int
                let replicaNum = (plan.FindPlanVertices pv).Count

                let prevPlanVerticesWithPolicyTag = 
                    policy.Edges |> Seq.filter (fun e -> e.Target = pv)
                ...
                let nextPlanVerticesWithPolicyTag = 
                    policy.Edges |> Seq.filter (fun e -> e.Source = pv)
                ...
                let BalancePolicyEdge (pe: IEdge<IPolicyVertex, IPolicyEdgeTag>) = 
                    let totalLoad = plan.FindPlanEdgeTags(pe.Tag) |> Seq.map (fun tag -> tag.Load)
                                                                  |> Seq.sum
